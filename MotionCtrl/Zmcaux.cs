...
*************************************************************/
        ...
        *************************************************************/
        ...
        public static Int32 ZAux_Direct_GetAxisEnable_new(IntPtr handle, int iaxis, ref int piValue)
        {
            int iresult;

            char[] cmdbuff = new char[2048];
            byte[] cmdbuffAck = new byte[2048];

            if (iaxis > MAX_AXIS_AUX || iaxis < 0)
            {
            ...
            string str_cmd = string.Format("?AXIS_ENABLE({0})", iaxis);
            iresult = ZAux_DirectCommand(handle, str_cmd, cmdbuffAck, 2048);
            if (ERR_OK != iresult)
            {
                return iresult;
            }
            ...
            string str = System.Text.Encoding.ASCII.GetString(cmdbuffAck);
            piValue = Convert.ToInt16(str);

            return ERR_OK;
        }
        public static Int32 ZAux_Direct_GetAxSta(IntPtr handle, int iaxis, ref List<int> list_value)
        {
            int iresult;
            ...
            //调用命令执行函数
            //iresult = ZAux_Execute(handle, cmdbuff, cmdbuffAck,2048);
            //string str_cmd = string.Format("CALL AX({0})", iaxis);
            string str_cmd = string.Format("?AXISSTATUS({0})\n?DPOS({0})\n?MPOS({0})\n?IN(0,31)\n?OP(0,31)\n?IN(32,63)\n?OP(32,63)\n", iaxis);
            iresult = ZAux_Execute(handle, str_cmd, cmdbuffAck, 2048);
            if (ERR_OK != iresult)
            {
                return iresult;
            }
            ...
            //sscanf(cmdbuffAck, "%d", piValue);
            string[] str = System.Text.Encoding.ASCII.GetString(cmdbuffAck).TrimEnd('\0').Split('\n');
        ...
        public static Int32 ZAux_Direct_GetIOSta(IntPtr handle, ref List<int> list_value)
        {
            int iresult;
            ...
            //iresult = ZAux_Execute(handle, cmdbuff, cmdbuffAck,2048);
            //string str_cmd = string.Format("CALL AX({0})", iaxis);
            string str_cmd = string.Format("?IN(0,31)\n?OP(0,31)\n?IN(32,63)\n?OP(32,63)\n");
            iresult = ZAux_Execute(handle, str_cmd, cmdbuffAck, 2048);
            if (ERR_OK != iresult)
            {
                return iresult;
